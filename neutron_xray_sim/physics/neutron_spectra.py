"""
neutron_xray_sim.physics.neutron_spectra
═════════════════════════════════════════
...
from .elements import ATOMIC_MASS, E0_THERMAL_MEV, NEUTRON_XS

# np.trapezoid exists from NumPy 2.0; np.trapz was removed in 2.0.
...
_E0_MEV  = E0_THERMAL_MEV   # thermal reference energy (meV)
_HBAR    = 1.054571817e-34  # J·s
_MN      = 1.674927471e-27  # neutron mass (kg)
_EV2J    = 1.602176634e-19  # eV → J

#: Bound-atom cross sections at 25.3 meV — shared with ``materials`` via
#: :mod:`neutron_xray_sim.physics.elements` (kept under this name for
#: backwards compatibility).
SIGMA_BOUND = NEUTRON_XS
...
class NeutronBeam:
    ...
    def __post_init__(self):
        ...
        if len(self.E_grid_meV) == 1:
            ...
            denom = float(_trapz(self.phi, self.E_grid_meV))
            self.E_mean_meV = float(
                _trapz(self.phi * self.E_grid_meV, self.E_grid_meV) / denom
            )
            lam = _energy_meV_to_lambda_angstrom(self.E_grid_meV)
            self.lambda_mean_angstrom = float(
                _trapz(self.phi * lam, self.E_grid_meV) / denom
            )
...
def cold_poly_beam(
    ...
    phi        /= _trapz(phi, E)  # normalise to 1
...
def mu_n_at_energy(
    ...
    for el, n in formula.items():
        if el not in SIGMA_BOUND:
            raise KeyError(
                f"Element '{el}' has no neutron cross sections. "
                f"Add it to neutron_xray_sim.physics.elements.NEUTRON_XS."
            )
        s = SIGMA_BOUND[el]
...
def mu_n_for_beam(
    ...
    return float(
        _trapz(beam.phi * mu_arr, beam.E_grid_meV)
        / _trapz(beam.phi, beam.E_grid_meV)
    )
